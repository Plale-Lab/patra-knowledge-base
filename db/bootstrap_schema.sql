...
--
-- events is keyed on (experiment_id, uuid), not uuid alone: image_generating_plugin
-- derives uuid deterministically from the image filename (uuid5(NAMESPACE_URL,
-- file_name)), so the same filename always produces the same uuid across different
-- experiments/users. A single-column UNIQUE(uuid) let the JDBC sink's upsert
-- silently overwrite an older experiment's row (including its experiment_id/
-- user_id) whenever a later experiment processed a same-named image.
DO $ckn$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'events_experiment_id_uuid_key') THEN
    ALTER TABLE events ADD CONSTRAINT events_experiment_id_uuid_key UNIQUE (experiment_id, uuid);
  END IF;
END
$ckn$;
...
-- Idempotent on re-delivery: JDBC sink upserts on (uuid, experiment_id); this
-- trigger runs on INSERT and UPDATE, and every insert it emits is ON CONFLICT
-- DO UPDATE/NOTHING.
